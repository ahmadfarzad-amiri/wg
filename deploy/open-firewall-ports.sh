...
Inbound UDP: ${_min}-${_max}  (or at minimum ${WG_CLIENT_PORT:-51820}$([[ -f /etc/wireguard/wg-tunnel.conf ]] && echo " and ${WG_TUNNEL_LISTEN_PORT}"))
TCP panels:  ${WG_PANEL_PORT:-8088}, ${WG_ADMIN_PORT:-8090} (if not using nginx)
Host iptables/ufw UDP ${WG_CLIENT_PORT:-51820} has been opened on this machine.
