...
if [[ -z "${WG_DEPLOY_REEXEC:-}" && ! -t 0 ]]; then
  ...
  GITHUB_RAW_BASE="${GITHUB_RAW_BASE:-https://cdn.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.13}"
...
EOF
  ...
  exit)
    if command -v ufw >/dev/null 2>&1; then
      ufw allow "${WG_TUNNEL_PORT}/udp" comment 'wg-tunnel exit' 2>/dev/null \
        || ufw allow "${WG_TUNNEL_PORT}/udp" || true
      log "ufw: allowed UDP ${WG_TUNNEL_PORT}"
    fi
    cat <<EOF
