...
| What | Source | Notes |
|------|--------|--------|
| `wg-ops` bootstrap + `pull` / `update` scripts | jsDelivr **pinned tag** (e.g. `@v1.0.13`) | Avoid `@latest` — jsDelivr purge is often throttled and can stick on an old release |
...
`https://cdn.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.13`
...
or https://cdn.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.13/deploy/config.env.example
...
1. Bump `GITHUB_CDN_REF` / `GITHUB_RAW_BASE` / docs URLs to the new tag (e.g. `v1.0.13`).
...
```bash
curl 'https://purge.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.13/deploy/wg-ops'
```
