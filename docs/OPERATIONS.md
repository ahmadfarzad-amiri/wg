...
| What | Source | Notes |
|------|--------|--------|
| `wg-ops` bootstrap + `pull` / `update` scripts | jsDelivr **pinned tag** (e.g. `@v1.0.12`) | Avoid `@latest` — jsDelivr purge is often throttled and can stick on an old release |
...
`https://cdn.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.12`

If `wg-ops update` still shows an old version (stuck `@latest` cache), force one pull:

```bash
sudo WG_RAW_BASE='https://cdn.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.12' wg-ops pull
```
...
or https://cdn.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.12/deploy/config.env.example
...
1. Bump `GITHUB_CDN_REF` / `GITHUB_RAW_BASE` / docs URLs to the new tag (e.g. `v1.0.12`).
...
```bash
curl 'https://purge.jsdelivr.net/gh/ahmadfarzad-amiri/wg@v1.0.12/deploy/wg-ops'
```
